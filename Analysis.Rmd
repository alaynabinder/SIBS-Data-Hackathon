...
fit.lasso <- glmnet(xx, yy, alpha=1, standardize=TRUE)
plot(fit.lasso, label=TRUE, xvar="lambda")
title(main="LASSO Regression for All ICU Data", line=3)
cbind(1:30,colnames(xx))
...
```{r final_models, echo=FALSE, warning=FALSE}
...
final_fit_stay <- glm(REC_IM ~ R_AB_3_n + NA_R_1_n + NA_R_2_n, data = imputed_data_stay, family = 'binomial')
