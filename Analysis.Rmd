...
my_function <- function(X, y, alpha = 1, lambda = NULL) {
  ...
  if(is.null(lambda)) {
    lambda <- glmnet::cv.glmnet(X, y, alpha = alpha, family = 'binomial')$lambda
...
imputed_data_admission2 = imputed_data_admission %>%
  dplyr::select(-c(REC_IM))

my_function(imputed_data_admission2, imputed_data_admission$REC_IM)
