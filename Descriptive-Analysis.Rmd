...
imputed_data_admisison <- imputed_data %>%
...
imputed <- subset(imputed_data, select = -c(.imp,.id,ritm_ecg_p_01, AGE, GB, S_AD_ORIT, ID)) 
...
fit_admission <- glm(REC_IM ~ ., 
           data = imputed_data_admisison, family = 'binomial')
t(t(vif(fit_admission)))

#Stay

fit_stay <- glm(REC_IM ~ .,
                data = imputed_data_stay, family= 'binomial')
t(t(vif(fit_stay)))
